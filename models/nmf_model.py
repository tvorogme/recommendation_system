...
features = lambda data: topics_similarity(data)
...
def generate_data(data):
    ...
    for urls in data.sequences.find().limit(-1):
        for url in urls['urls']:
            ...
            print(_id)
            x.append(features(_id))
        y.append(1)
    print('Start generate random data.')
    len_true_data = len(x)
    for a in range(0, len_true_data):
        x.append(features(data.get_random_articles()))
        y.append(0)
